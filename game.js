...
class Game {
    ...
    update_instructions()
    {
        if (this.is_first_input) 
        {
            ...
            this.instruction = `You are the narrator of an RPG game. Start a story based on this context: "${this.context}". Integrate this puzzle: "${generate_cipher.key}", in your text.After that You must absolutely Provide 3 choices A) B) or C) at the end of the paragraph and the third choice must be: "C) Guess the word." The story should be in a natural, paragraph form, not a code or image cipher and should be short (2 sentences max) but still have the 3 choices that I talked about.`;
        }

        else {
            this.instruction = `You are continuing the RPG story from the context: "${this.context}". The player has already seen the following summary: "${summarize_output(generate_text.output)}" and their input was: "${this.input}". Now, continue the story, but be sure to include the puzzle: "${generate_cipher.key}", to your text. Again after that, you must absolutely provide 3 choices A) B) or C)  at the end and the third choice must be: "C) Guess the word."The story should be in a natural, paragraph form, not a code or image cipher and should be short (2 sentences max) but still have the 3 choices that I talked about.;`;
        }
    ...
    check_game_over()
    {
        ...
        else if (this.player.attempt && this.input.toLowerCase() == this.magic_word.toLowerCase())
        {
